...
class Plotter(object):
    ...
    def __init__(self, x, z=None, function=None, **kwds):
        ...
        if type(function) is str:
            from interpf import interpf_nd as interpf #NOTE: axis may change
...
#        figure = plt.figure()
       ...
       #fix = np.zeros(len(ix))
        ...
        x_ = grid[ax0]
        y_ = grid[ax1]
        ax.plot_wireframe(x_, y_, z_, rstride=d, cstride=d, alpha=0.1, color = 'green',linewidth = 1,label='Surrogate')
#        surf=ax.plot_surface(x_*0.012, y_*0.012, z_, rstride=d, cstride=d, cmap=cm.plasma, linewidth=1, antialiased=False, label='Surrogate')
        ax.grid(False)
        ax.view_init(elev=37, azim=5)
        ...
        y_ = x.T[ax1]
        N=2
        ax.plot(x_, y_, z_, 'bo', linewidth=2, markersize=3, label='Simulation')
        
#        print('len', len(x_))
#        ax.set_zlabel('pressure [MeV fm$^{-3}$]',fontsize=15, labelpad=15)
                #        ax.set_zlabel('$\\beta H(r)/\Gamma$',fontsize=15, labelpad=40)
        #        ax.set_zlim(-1,2)
#        ax.set_ylabel('density [fm$^{-3}$]',fontsize=15, labelpad=10)
#        ax.set_xlabel('proton fraction',fontsize=15, labelpad=10)
#        surf._facecolors2d=surf._facecolors3d
#        surf._edgecolors2d=surf._edgecolors3d
#        ax.legend(loc='best')
#        ax.legend(bbox_to_anchor=(0.5, .80), loc=2, borderaxespad=0.)

#        plt.savefig('EOS_surface.png',dpi=199)
