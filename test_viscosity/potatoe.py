...
#  - file://LICENSE
#  - import potatoe; potatoe.license()
"""
copied from potatoe/_robert.py
"""
...
def _data(name):
    m = Monitor()
    m._x, m._y = ds.read_archive(name)
    n = Monitor()
    n._x, n._y = ds.read_archive(name+'i')
    m = m+n
    del n
    from mystic.math.legacydata import dataset
    data = dataset().load(m._x, m._y, ids=list(range(len(m))))
    return data

def _interp(data):
    return ds.interpolate(data, method='thin_plate')
"""
