...
def _getitem(data, axis):
    ...
    """
    if len(data.values) and type(data.values[0]) is not tuple: #XXX: or list]?
...
def interpolate(data, step=None, **kwds):
    ...
    if axis is None:
        if len(data.values) and type(data.values[0]) is tuple: #XXX: or list]?
...
#FIXME: the following assume archive built with Searcher... still valid?
#FIXME: skipped... is this still relevant for 3d?
def load_archive(filename, tol=1, cached=False): #XXX: or accept archive?
    """load an klepto.archives.dir_archive written with mystic.search.Searcher

    Inputs:
      filename: str path to location of klepto.archives.dir_archive
      tol: int, tolerance for rounding (default is 1)
      cached: if True, build a cached archive

    NOTE:
      builds an inf_cache with keymap() and ignore=('**','out')
    """
    import klepto as kl
    ca = kl.archives.dir_archive(filename, cached=cached)
    k = keymap=kl.keymaps.keymap()
    c = kl.inf_cache(keymap=k, tol=tol, ignore=('**','out'), cache=ca)
    memo = c(lambda *args, **kwds: kwds['out'])
    cache = memo.__cache__()
    return cache
...
#FIXME: skipped... is this still relevant for 3d?
def as_cache(data, cache=None, tol=None): 
    '''convert mystic.math.legacydata.dataset to klepto.archive
    ...
    import klepto as kl
    if cache is None:
        cache = kl.archives.null_archive()
    elif type(cache) is dict:
        cache = kl.archives.null_archive(dict=cache)
    #NOTE: assumes 'y' ignored; should 'y' be ignored?
    #NOTE: uses *args, **kwds, where 'out' in kwds
    k = keymap=kl.keymaps.keymap()
    c = kl.inf_cache(keymap=k, tol=None, ignore=('**','out'), cache=cache)
    memo = c(lambda *args, **kwds: kwds['out'])
    f = lambda x,y,id: memo(*x, **dict(out=y, id=id))
    list(map(f, data.coords, data.values, data.ids))
    cache.update(memo.__cache__())
    return cache


"""
def to_dataset(cache, data=None):
    '''convert a klepto.archive to a mystic.math.legacydata.dataset'''
    #NOTE: assumes archive used klepto.keymaps.stringmap (as in `to_cache`)
    #NOTE: assumes archive used function with ('x','y','id'), ignored 'y'
    import numpy as np
    if data is None:
        from mystic.math.legacydata import dataset
        data = dataset()
    y = [eval(k.replace('NULL',str(v))) for (k,v) in cache.items()]
    if len(y) is 0:
        return data
    ix = np.array(y, dtype=object)[:,::2][0]
    y = np.array(y, dtype=object)[:,1::2].T
    return data.load(y[ix=='x'][0].tolist(), y[ix=='y'][0].tolist(), y[ix=='id'][0].tolist())


def to_cache(data, cache=None): 
    '''convert mystic.math.legacydata.dataset to klepto.archive'''
    import klepto as kl
    if cache is None:
        cache = kl.archives.null_archive()
        ...
        cache = kl.archives.null_archive(dict=cache)
    #NOTE: assumes 'y' ignored; should 'y' be ignored?
    #NOTE: uses 'x','y','id'... are these the desired names???
    c = kl.inf_cache(ignore=('y'), keymap=kl.keymaps.stringmap(), cache=cache)
    f = c(lambda x,y=kl.NULL,id=kl.NULL: y)
    list(map(f, data.coords, data.values, data.ids))
    cache.update(f.__cache__())
    return cache


if __name__ == '__main__':
    from mystic.math.legacydata import dataset, load_dataset
    d = load_dataset('xxx.txt')
    a = to_dataset(to_cache(d)) 
    assert sorted(a) == sorted(d)

    from copy import copy
    a = to_dataset(to_cache(dataset()), copy(d))
    assert sorted(a) == sorted(d)

    a = to_dataset(to_cache(dataset(), to_cache(d).copy()))
    assert sorted(a) == sorted(d)
"""
