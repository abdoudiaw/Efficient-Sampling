'''
We define: dist = F(param)
where dist = distance(data, function=fi, axis=i)
where fi = lambda *x: mi.predict(x)
where mi = e(param).fit(x,y)

We can: find min of dist = F(param) for a given set of data (x,y).
This gives: param that minimizes the dist, with no stated uncertatinty.

We can: find min E dist = F(param) for a random set of data (x,y).
This gives: param that minimizes the E|dist|.

We can: find min max E dist = F(params) for a random set of data (x,y).
This gives: param that minimizes the max E|dist|.


def doit(param, train, test):
  *) build model = e(param).fit(train)
  *) find score for func = model.predict(test)
  *) find dist for data,func
  return dist

def average(param):
  *) build N splits of the data: train,test = data
  *) find dist for each of the N splits
  return mean(dist)

def range(param):
  *) build N splits of the data: train,test = data
  *) find dist for each of the N splits
  return spread(dist) # or variance(dist)

MIN: param* = solver(average)
MIN: param* = solver(range)

For sampling the data, use Monte Carlo or product_measure.sampled_support
''' #NOTE: this script should be a class
...
def measure(xtrain, xtest, ytrain, ytest, **param): #XXX: dataset? monitor?
    ...
    vscore = lambda i,j,k: score(i,j,k, delta=delta, tries=tries, verbose=verbose)
    ...
    #data = ds.from_archive(ds.load_archive(mname), axis=None)
