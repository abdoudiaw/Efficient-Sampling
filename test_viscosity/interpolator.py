...
class Interpolator(object):
   ...
   #    _downsample - skip sampled points at a regular interval (for speed)
    ...
    def _interpolate(self, x, z, **kwds):
        ...
        """
        import numpy as np
        from interpf import interpf_nd as interpf
    ...
    def Interpolate(self, **kwds): #XXX: better take a strategy?
        ...
        # build the surrogate
        retry = 0 #FIXME: don't hardwire, but do move noise into retry loop
        if not retry:
            self.function = self._interpolate(x, z, **args)
            return self.function
        #XXX: the following is unnessary, unless add random (noise) each time
        import numpy as np
        from mystic.math.interpolate import _to_objective
        from mystic.math.distance import graphical_distance
        from mystic.math.legacydata import dataset
        count = -1 #FIXME: didn't update/test the following for multi-valued z
        best = np.inf
        data = dataset().load(x,z,ids=list(range(len(x))))
        while retry > count: # stop after retry with no better fit
            function = self._interpolate(x, z, **args)
            dist = np.nanmean(graphical_distance(_to_objective(function),data))
            print('dist: {} @ try {}'.format(dist, count+1))
            if dist < best:
                best = dist
