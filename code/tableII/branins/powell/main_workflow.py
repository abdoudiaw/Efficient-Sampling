...
def valid(dist, max=maxtol, sum=sumtol):
...
def validate(x, z, axis, data=None, func=None, **kwds):
    ...
    if func is None:
        ...
        while not valid(dist) and repeat:
            ...
            repeat -= 1
        if valid(dist):
            print('train valid: max:{max}, sum:{sum}'.format(max=dist.max(), sum=dist.sum()))
            return f, dist, None
        #NOTE: this code should be reached only very rarely if at all
        print('train invalid: max:{max}, sum:{sum}'.format(max=dist.max(), sum=dist.sum()))
        # launch searchers
        s = sum(s.evals() for s in search(axis)) #XXX: Pool().map?
        fcalls[0] += s
        print('{a}: evals:{e}, cache:{c}'.format(a=axis,e=s,c=len(c)))
        ...
        if not valid(dist):
            ...
            print('test invalid: max:{max}, sum:{sum}'.format(max=dist.max(), sum=dist.sum()))
            ...
            del _data
            #print('new invalid: max:{max}, sum:{sum}'.format(max=dist.max(), sum=dist.sum())) #NOTE: generally should be identical to 'test invalid'
            res = validate(x,z, axis=axis, data=data, func=None, **kwds)
            return res[0], res[1], dist
        print('test valid: max:{max}, sum:{sum}'.format(max=dist.max(), sum=dist.sum()))
...
it = fails # this is number of refit tries before 'fail' to sampling
w = warms  # this is size of 'warm'
...
def xxx(i):
    ...
    if _t is not None and _d is not None:
        hist[i].append(_t.sum()) #FIXME: mean or max? or sum?
...
null = float('nan')
tol = reptol # stop when rep converges so rep iters <= reptol
rep = nrep # this is hist.values()[-rep:] <= reptol
last = nlast # this is any(size[-nlast:])
loops = maxloops # max number of loops
done = False
start = loops
...
print('total evals: {e}'.format(e=fcalls[0]))
