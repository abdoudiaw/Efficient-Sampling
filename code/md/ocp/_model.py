...
#lower = 0.04 # lower bound on input parameters
#upper = 1.5 # upper bound on input parameters
bounds = [(0.05,5.0),(0.1,150)]  #* shape[0] # bounds (lower is 0.012) on input parameters

# define hypercube for plotting
#hcube = '{0}:{1}:{2}, {0}:{1}:{2}'.format(lower, upper, upper/50.)
hcube='0.05:5:0.05, 0.1:150:1.5'
hcube += ''
...
reptol = 2e-4 #2e-6 # stop when rep converges so rep iters <= reptol
nrep = 1 #3 # this is hist.values()[-rep:] <= reptol
nlast = 1 #3 # this is any(size[-nlast:])
maxloops = 1 #30 # max number of loops

# objective
from objective import OCP as model
#from objective import gstr as model
